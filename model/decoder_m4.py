...
class ECAM(nn.Module):
    def __init__(self, dim_edge, dim_region, out_dim):
        super().__init__()

        self.edge_proj = nn.Sequential(
            nn.Conv2d(dim_edge, out_dim, 1),
            nn.BatchNorm2d(out_dim),
            nn.ReLU(inplace=True)
        )

        self.region_proj = nn.Sequential(
            nn.Conv2d(dim_region, out_dim, 1),
            nn.BatchNorm2d(out_dim),
            nn.ReLU(inplace=True)
        )
    ...
    def forward(self, edge_feature, region_feature):
        # ============================================================
        # 关键：Region → Edge 尺度对齐（输出以 Edge 为准）
        # ============================================================
        if region_feature.shape[-2:] != edge_feature.shape[-2:]:
            region_feature = F.interpolate(
                region_feature,
        ...
        # ===== Channel projection =====
        edge = self.edge_proj(edge_feature)      # [B, C, 8, 8]
        region = self.region_proj(region_feature)  # [B, C, 8, 8]

        # ===== Edge-guided modulation =====
        gate = self.edge_gate(edge)              # [B, 1, 8, 8]
        region_guided = region * gate

        # ===== Collaborative fusion =====
        fused = self.fusion(
            torch.cat([region, region_guided], dim=1)
        )

        # ===== Boundary refinement =====
        out = self.boundary_refine(fused)
...
class Decoder(nn.Module):
    def __init__(self, channels):
        ...
        self.fuse1 = nn.Sequential(nn.Conv2d(channels, channels, kernel_size=3, stride=1, padding=1, bias=False),nn.BatchNorm2d(channels))
        self.fuse2 = nn.Sequential(nn.Conv2d(channels, channels, kernel_size=3, stride=1, padding=1, bias=False),nn.BatchNorm2d(channels))
        self.fuse3 = nn.Sequential(nn.Conv2d(channels, channels, kernel_size=3, stride=1, padding=1, bias=False),nn.BatchNorm2d(channels))
        ...
        # self.MSA5_1=MSA_module(dim = channels)
        # self.MSA4_1=MSA_module(dim = channels)
        # self.MSA3_1=MSA_module(dim = channels)
        # self.MSA2_1=MSA_module(dim = channels)
        self.CAM1 = ECAM(64, 128,128)
        self.CAM2 = ECAM(128, 128,128)
        self.CAM3 = ECAM(320, 128,128)
        self.CAM4 = ECAM(512, 128,128)

        # self.cam1 = CAM(64, 128)
        # self.cam2 = CAM(128, 128)
        # self.cam3 = CAM(320, 128)
        # self.cam4 = CAM(512, 128)
    ...
    def forward(self, E4, E3, E2, E1,shape):
        ...
        E4 = F.relu(self.fuse1(E4), inplace=True) #对融合后的特征做非线性增强，使卷积（fuse）后的特征更稳定、更具有表达能力
        ...
        D4 = self.MSA5(E5, P5)
        # D4_1 = self.MSA5_1(E4, P5)
        #torch.Size([2, 512, 8, 8]) torch.Size([2, 128, 16, 16]) 128 16
        D41= self.CAM4(s4,D4)
        D411 = F.interpolate(D4, size=E3.size()[2:], mode='bilinear')
        
        P4  = self.predtrans4(D411)
        
        D3 = self.MSA4(D411, P4)
        #D3_1 = self.MSA4_1(E3, P4)
        #torch.Size([2, 320, 16, 16]) torch.Size([2, 128, 16, 16])  128 16
        D31= self.CAM3(s3,D3) 
        D311 = F.interpolate(D3,   size=E2.size()[2:], mode='bilinear')
        P3  = self.predtrans3(D311)  
        
        D2 = self.MSA3(D311, P3)
        # D2_1 = self.MSA3_1(E2, P3)  128 64
        D21= self.CAM2(s2,D2)
        D211 = F.interpolate(D2, size=E1.size()[2:], mode='bilinear')
        P2  = self.predtrans2(D211)
        
        D1 = self.MSA2(D211, P2)
        # D1_1 = self.MSA2_1(E1, P2)
        D11= self.CAM1(s1,D1)
        P1  =self.predtrans1(D11)
        ...
        return P5,P4,P3,P2,P1, D41, D31, D21, D11
